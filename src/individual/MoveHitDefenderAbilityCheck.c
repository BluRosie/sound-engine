...
BOOL MoveHitDefenderAbilityCheckInternal(struct BattleSystem *bw, struct BattleStruct *sp, int *seq_no)
{
    ...
    if (MoldBreakerAbilityCheck(sp, sp->attack_client, sp->defence_client, ABILITY_STATIC)) {
        if ((sp->battlemon[sp->attack_client].hp)
            ...
            seq_no[0] = BATTLE_SUBSCRIPT_APPLY_PARALYSIS;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->attack_client].hp)
            ...
            case 0:
            default:
                seq_no[0] = BATTLE_SUBSCRIPT_APPLY_POISON;
                break;
            case 1:
                seq_no[0] = BATTLE_SUBSCRIPT_APPLY_PARALYSIS;
                break;
            case 2:
                seq_no[0] = BATTLE_SUBSCRIPT_APPLY_SLEEP;
                break;
            }
            sp->addeffect_type = ADD_STATUS_ABILITY;
            sp->state_client = sp->attack_client;
        ...
        if ((sp->battlemon[sp->attack_client].hp)
            ...
            sp->state_client = sp->attack_client;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_APPLY_BURN;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->attack_client].hp)
            ...
            sp->state_client = sp->attack_client;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_APPLY_ATTRACT;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->defence_client].hp == 0)
            ...
            sp->battlerIdTemp = sp->attack_client;
            seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_AFTERMATH;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->defence_client].hp == 0)
            ...
            sp->battlerIdTemp = sp->attack_client;
            seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_INNARDS_OUT_MESSAGE;
            ret = TRUE;
        }
    }
    // handle rattled
        ...
        if ((sp->battlemon[sp->defence_client].hp)
            ...
            if ((movetype == TYPE_FIRE) || (movetype == TYPE_WATER)) {
                sp->addeffect_param = MOVE_SUBSCRIPT_PTR_BOOST_STATS_SPEED_UP_6;
                sp->addeffect_type = SIDE_EFFECT_TYPE_ABILITY;
                sp->state_client = sp->defence_client;
                sp->battlerIdTemp = sp->defence_client;
                seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_ABILITY_STAT_CHANGE;
        ...
        if (/*(sp->battlemon[sp->defence_client].hp) // mon notably does not need to be alive for this ability to proc
            && */
            (sp->oneSelfFlag[sp->defence_client].physical_damage) || (sp->oneSelfFlag[sp->defence_client].special_damage)) {
            sp->addeffect_param = MOVE_SUBSCRIPT_PTR_CHARGE;
            sp->addeffect_type = SIDE_EFFECT_TYPE_ABILITY;
            sp->state_client = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_CHARGE_BOOST;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->defence_client].hp)
            ...
            if (movetype == TYPE_WATER) {
                if (sp->battlemon[sp->defence_client].states[STAT_DEFENSE] < 11) {
                    sp->addeffect_param = MOVE_SUBSCRIPT_PTR_DEFENSE_UP_2_STAGES;
                    sp->addeffect_type = SIDE_EFFECT_TYPE_ABILITY;
                    sp->state_client = sp->defence_client;
                    sp->battlerIdTemp = sp->defence_client;
                    seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_ABILITY_STAT_CHANGE;
        ...
        if (((sp->moveStatusFlagForSpreadMoves[sp->defence_client] & WAZA_STATUS_FLAG_CRITICAL) != 0)
            ...
            sp->state_client = sp->defence_client;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_ANGER_POINT;
            ret = TRUE;
        }
        ...
        if ((sp->battlemon[sp->defence_client].hp)
            ...
            sp->battlerIdTemp = sp->defence_client;
            sp->addeffect_type = SIDE_EFFECT_TYPE_ABILITY;
            seq_no[0] = BATTLE_SUBSCRIPT_HANDLE_WEAK_ARMOR;
            ret = TRUE;
        }
    }
    // handle cursed body - disable the last used move by the pokemon.  disabling is handled here, script just displays the message
        ...
        if ((sp->oneSelfFlag[sp->defence_client].physical_damage) || (sp->oneSelfFlag[sp->defence_client].special_damage)) {
            ...
            sp->state_client = sp->defence_client;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_SAND_STREAM;
            ret = TRUE;
        }
                ...
                (sp->oneSelfFlag[sp->defence_client].special_damage)))
        {
            ...
            sp->state_client = sp->defence_client;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_CREATE_TERRAIN_OVERLAY;
            ret = TRUE;
        }
            ...
            && (sp->oneSelfFlag[sp->defence_client].physical_damage))
        {
            ...
            if (sp->scw[fieldSide].toxicSpikesLayers < 2) {
                ...
                seq_no[0] = BATTLE_SUBSCRIPT_TOXIC_DEBRIS;
                ret = TRUE;
            }
        }
        ...
        if (sp->oneSelfFlag[sp->defence_client].physical_damage || sp->oneSelfFlag[sp->defence_client].special_damage) {
            ...
            sp->addeffect_type = SIDE_EFFECT_TYPE_PRINT_WORK_ABILITY;
            sp->battlerIdTemp = sp->defence_client;
            seq_no[0] = BATTLE_SUBSCRIPT_COTTON_DOWN;
            ret = TRUE;
        }
