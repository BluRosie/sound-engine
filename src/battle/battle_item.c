    ...
    *seq_no = 0;
    ...
    case HOLD_EFFECT_STATUS_RESTORE:
        if ((ctx->battlemon[battlerId].condition & STATUS_ALL) || (ctx->battlemon[battlerId].condition2 & STATUS2_CONFUSION)) {
            ...
            if ((ctx->battlemon[battlerId].condition & STATUS_ALL) && (ctx->battlemon[battlerId].condition2 & STATUS2_CONFUSION)) {
                *seq_no = BATTLE_SUBSCRIPT_ITEM_RECOVER_ALL;
            }
        }
        break;
    default:
    ...
    if (ctx->battlemon[battlerId].hp) {
        ...
        case HOLD_EFFECT_HP_RESTORE: // oran berry, berry juice
            if (hpLowerThan50) {
                ...
                script = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE;
                ret = TRUE;
            }
            break;
        ...
        case HOLD_EFFECT_PP_RESTORE: // leppa berry
        {
            ...
            if (index != CLIENT_MAX) {
                ...
                script = BATTLE_SUBSCRIPT_ITEM_PP_RESTORE;
                ret = TRUE;
            }
            break;
        }
        ...
        case HOLD_EFFECT_HP_RESTORE_SPICY: // figy berry
            if (hpLowerThan50) {
                ...
                if (GetFlavorPreferenceFromPID(ctx->battlemon[battlerId].personal_rnd, FLAVOR_SPICY) == -1) {
                    script = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE_CNF;
                } else {
                    script = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE;
                }
                ret = TRUE;
        ...
        case HOLD_EFFECT_PINCH_ATK_UP: // liechi berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && ctx->battlemon[battlerId].states[1] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                ctx->msg_work = 1;
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
                ret = TRUE;
            }
            break;
        case HOLD_EFFECT_PINCH_DEF_UP: // ganlon berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && ctx->battlemon[battlerId].states[2] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                ctx->msg_work = 2;
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
                ret = TRUE;
            }
        ...
        case HOLD_EFFECT_PINCH_SPEED_UP: // salac berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && ctx->battlemon[battlerId].states[3] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                ctx->msg_work = 3;
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
                ret = TRUE;
            }
        ...
        case HOLD_EFFECT_PINCH_SPATK_UP: // petaya berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && ctx->battlemon[battlerId].states[4] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                ctx->msg_work = 4;
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
                ret = TRUE;
            }
        ...
        case HOLD_EFFECT_PINCH_SPDEF_UP: // apicot berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && ctx->battlemon[battlerId].states[5] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                ctx->msg_work = 5;
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
                ret = TRUE;
            }
        ...
        case HOLD_EFFECT_PINCH_CRITRATE_UP: // lansat berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost && !(ctx->battlemon[battlerId].condition2 & STATUS2_FOCUS_ENERGY)) {
                script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST_2;
                ret = TRUE;
            }
            break;
        case HOLD_EFFECT_PINCH_RANDOM_UP: // starf berry
            ...
            if (ctx->battlemon[battlerId].hp <= (s32)(ctx->battlemon[battlerId].maxhp) / boost) {
                ...
                for (stat = 0; stat < 5; stat++) {
                    if (ctx->battlemon[battlerId].states[1 + stat] < BATTLE_SUBSCRIPT_BOOST_STATS) {
                        break;
                    }
                }
                if (stat != 5) {
                    ...
                    } while (ctx->battlemon[battlerId].states[1 + stat] == BATTLE_SUBSCRIPT_BOOST_STATS);
                    ctx->msg_work = stat + 1;
                    script = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST_2;
                    ret = TRUE;
                }
        ...
        case HOLD_EFFECT_HEAL_MENTAL_CONDITIONS: // Mental Herb
            ...
            || ctx->battlemon[battlerId].moveeffect.disabledTurns) {
                script = BATTLE_SUBSCRIPT_ITEM_HEAL_MENTAL_CONDITIONS;
                ret = TRUE;
            }
            break;
        case HOLD_EFFECT_PINCH_ACC_UP: // micle berry
            ...
            if (ctx->battlemon[battlerId].hp <= ((s32)(ctx->battlemon[battlerId].maxhp) / boost)) {
                script = BATTLE_SUBSCRIPT_ITEM_ACC_UP_ONCE;
                ret = TRUE;
            }
            break;
        default:
...
BOOL LONG_CALL TryFling(struct BattleSystem *bsys, struct BattleStruct *sp, int battlerId)
{
    ...
    case STEAL_EFFECT_RESTORE_HP: // Oran Berry
        sp->flingData = mod;
        sp->flingScript = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE;
        break;
    ...
    case STEAL_EFFECT_CURE_ALL: // Lum Berry
        if ((sp->battlemon[sp->defence_client].condition & STATUS_ALL) || (sp->battlemon[sp->defence_client].condition2 & STATUS2_CONFUSION)) {
            ...
            if ((sp->battlemon[sp->defence_client].condition & STATUS_ALL) && (sp->battlemon[sp->defence_client].condition2 & STATUS2_CONFUSION)) {
                sp->flingScript = BATTLE_SUBSCRIPT_ITEM_RECOVER_ALL;
            }
        }
        break;
    case STEAL_EFFECT_RESTORE_SPICY: // Figy Berry
        ...
        if (GetFlavorPreferenceFromPID(sp->battlemon[sp->defence_client].personal_rnd, FLAVOR_SPICY) == -1) {
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE_CNF;
        } else {
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_HP_RESTORE;
        }
        break;
    ...
    case STEAL_EFFECT_RESET_STATS: // White Herb
    {
        ...
        for (stat = 0; stat < 8; stat++) {
            if (sp->battlemon[sp->defence_client].states[stat] < 6) {
                ...
                sp->flingScript = BATTLE_SUBSCRIPT_ITEM_RECOVER_STAT_DROP;
            }
        }
    } break;
    case STEAL_EFFECT_CURE_MENTAL_CONDITIONS: // Mental Herb
        if (sp->battlemon[sp->defence_client].condition2 & STATUS2_ATTRACT
            ...
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_HEAL_MENTAL_CONDITIONS;
        }
        break;
    case STEAL_EFFECT_FLINCH: // King's Rock, Razor Fang
        sp->state_client = battlerId;
        sp->addeffect_type = SIDE_EFFECT_TYPE_INDIRECT;
        sp->flingScript = BATTLE_SUBSCRIPT_TRY_FLINCH;
        break;
    case STEAL_EFFECT_PARALYZE: // Light Ball
        sp->state_client = battlerId;
        sp->addeffect_type = SIDE_EFFECT_TYPE_INDIRECT;
        sp->flingScript = BATTLE_SUBSCRIPT_APPLY_PARALYSIS;
        break;
    case STEAL_EFFECT_POISON: // Poison Barb
        sp->state_client = battlerId;
        sp->addeffect_type = SIDE_EFFECT_TYPE_INDIRECT;
        sp->flingScript = BATTLE_SUBSCRIPT_APPLY_POISON;
        break;
    ...
    case STEAL_EFFECT_BURN: // Flame Orb
        sp->state_client = battlerId;
        sp->addeffect_type = SIDE_EFFECT_TYPE_INDIRECT;
        sp->flingScript = BATTLE_SUBSCRIPT_APPLY_BURN;
        break;
    ...
    case STEAL_EFFECT_DEF_UP: // Ganlon Berry, Kee Berry
        if (sp->battlemon[sp->defence_client].states[STAT_DEFENSE] < 12) {
            ...
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST;
        }
        break;
    ...
    case STEAL_EFFECT_RANDOM_UP: // Starf Berry
    {
        ...
        if (stat != 5) {
            ...
            sp->msg_work = stat + 1;
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_STAT_BOOST_2;
        }
        break;
    }
    case STEAL_EFFECT_CRITRATE_UP: // Lansat Berry
        if (!(sp->battlemon[sp->defence_client].condition2 & STATUS2_FOCUS_ENERGY)) {
            sp->flingScript = BATTLE_SUBSCRIPT_ITEM_RAISE_CRIT;
        }
        break;
    case STEAL_EFFECT_ACC_UP: // Micle Berry
        sp->flingScript = BATTLE_SUBSCRIPT_ITEM_ACC_UP_ONCE;
        break;
    default:
        break;
    }
